...
KCONFIGS2="--regex='/^[ \t]*\(\(menu\)*config\)[ \t]+\([a-zA-Z0-9_]+\)/\3/'"
...
all_target_sources $2 | cscope -bkq -i -

# make ctags from current director
all_target_sources | ctags -V --file-scope=yes --langmap=c:+.h --languages=c,c++ --links=yes --c-kinds=+p --c++-kinds=+p --fields=+iaS --extra=+q -L - \
